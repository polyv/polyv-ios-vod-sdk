...
@implementation PLVVodMarqueeLabel
...
+ (void)controllerViewWillAppear:(UIViewController *)controller {
    [PLVVodMarqueeLabel restartLabelsOfController:controller];
}
...
#pragma mark - PLVVodMarqueeLabel Heavy Lifting
...
- (void)setNumberOfLines:(NSInteger)numberOfLines {
    // By the nature of PLVVodMarqueeLabel, this is 1
    [super setNumberOfLines:1];
}

- (void)setAdjustsFontSizeToFitWidth:(BOOL)adjustsFontSizeToFitWidth {
    // By the nature of PLVVodMarqueeLabel, this is NO
...
- (void)setMarqueeType:(PLVVodMarqueeType)marqueeType {
...
- (CGFloat)solveTForY:(CGFloat)y_0 withEpsilon:(CGFloat)epsilon controlPoints:(NSArray *)controlPoints
{
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (fabs(df0) < 1e-6) {
            NSLog(@"PLVVodMarqueeLabel: Newton's Method failure, small/zero derivative!");
            break;
        }
        // Else recalculate t1
        t1 = t0 - f0/df0;
    ...
    NSLog(@"PLVVodMarqueeLabel: Failed to find t for Y input!");
    return t0;
}
